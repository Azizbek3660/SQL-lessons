...
select name, substring(name,1,CHARINDEX(',',name)-1)as Name,substring(name,CHARINDEX(',',name)+1,len(name))as Surname from TestMultipleColumns
...
SELECT TRANSLATE('1234ABC123456XYZ1234567890ADS', '0123456789', REPLICATE('X', 10)) AS Result
...
select*,len(texts)-len(REPLACE(texts,' ',''))  AS COUNT from CountSpaces
...
FROM 
    Employees
WHERE 
    FLOOR(DATEDIFF(CURDATE(), HIRE_DATE) / 365) > 10 
    AND FLOOR(DATEDIFF(CURDATE(), HIRE_DATE) / 365) < 15;
...
FROM 
  ...
  string_agg (sdgfhsdgfhs@121313131,
...
select*from p1
select*from p2
select p1.id,p2.id,replace(p1.code,0,5) as new_code from p1
join p2
on p1.id=p2.id
...
select*from Employees
select EMPLOYEE_ID,
YEAR(getdate()) - YEAR(HIRE_DATE) as DATE ,
case when HIRE_DATE  < 1 then 'new hire' 
     when HIRE_DATE between 1 and 5 then 'junior'
	 when HIRE_DATE between 5 and 10  then 'mid-level'
	 when HIRE_DATE between 10 and 20  then 'senior'
	 else 'veteran'
	 end 
from Employees
WHERE  HIRE_DATE=YEAR(getdate()) - YEAR(HIRE_DATE)

15-Write a SQL query to extract the integer value that appears at the start of the string in a column named Vals.(GetIntegers)
SELECT 
    Id,
    VALS,
    CASE 
...
16-In this puzzle you have to swap the first two letters of the comma separated string.(MultipleVals)
...
SELECT 
  ...
  STUFF(
    STUFF(Vals, 1, CHARINDEX(',', Vals), SUBSTRING(Vals, CHARINDEX(',', Vals) + 1, CHARINDEX(',', Vals, CHARINDEX(',', Vals) + 1) - CHARINDEX(',', Vals) - 1)),
    CHARINDEX(',', Vals) + 1,
...
17-Write a SQL query that reports the device that is first logged in for each player.(Activity)
SELECT 
    a.player_id,
    a.device_id
FROM 
...
ON 
    a.player_id = first_logins.player_id AND
    a.event_date = first_logins.first_login;

	18-You are given a sales table. Calculate the week-on-week percentage of sales per area for each financial week. For each week, the total sales will be considered 100%, and the percentage sales for each day of the week should be calculated based on the area sales for that week.(WeekPercentagePuzzle)
