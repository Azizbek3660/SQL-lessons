...
from [master].[dbo].[Products_Discounted]
...
9-Use SELECT DISTINCT on two columns (Category and ProductName) in the Products table.
 ...
 12-Use COALESCE to return the first not NULL value from FirstName or LastName in the Employees table.
  ...
  14-Write a query that filters the Employees table to show employees whose Age is either between 30 and 40 or Department = 'Marketing'.
   ...
   SELECT TOP (1000) [EmployeeID]
      ...
      ,[Country]
  FROM [master].[dbo].[Employees]
  where age in (30,40) and DepartmentName='marketing'
...
16-Write a query to display all products with Price <= 1000 and Stock > 50, sorted by Stock in ascending order
 ...
 SELECT  [ProductID]
      ...
      ,[StockQuantity]
  FROM [master].[dbo].[Products]
  where price <=10000 and stockquantity > 50 
...
SELECT  [ProductID]
  ...
  SELECT TOP (1000) [EmployeeID]
      ...
      ,[Country]
  FROM [master].[dbo].[Employees]
  where DepartmentName IN ('HR','IT','FINSNCE')
