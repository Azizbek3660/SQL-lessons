...
4-Write a query to count the number of unique product categories (COUNT(DISTINCT Category)) from the Productss table.
...
select saleID,sum ([SaleAmount])from sales
group by saleID
having saleID=7
...
8-Write a query to show the minimum and maximum Price of productss grouped by Category. Use productss table.

select min(price)[min],
       max (price)[max]from productss
...
select sum (saleamount)[sum],
       avg (saleamount)[AVG]from sales
group by productid
...
select departmentname,avg (salary)[avg salary] from employees
group by departmentname
...
select*from employees
select DepartmentName,sum (salary) [sum salary]from employees
group by DepartmentName
...
select departmentname,sum(salary)[sum salary],
       avg(salary)[avg salary]from Employees
	   group by departmentname
...
select  year (orderdate),min(quantity)[min],max(quantity)[max]from Orders
group by OrderDate
