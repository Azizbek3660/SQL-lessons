...
select  CONCAT(EMPLOYEE_ID,'-',FIRST_NAME,' ',LAST_NAME) as[full name] from Employees
...
select manager_id, sum((salary)) as [max_salary] from Employees
group by MANAGER_ID
...
select max(Max1) as[max],max(Max2)as[max2],max(Max3)as [max3] from TestMax



6-Find me odd numbered movies and description is not boring.(cinema)
select id,description from cinema
where id % 2=1
...
select id,coalesce(ssn,passportid,itin)as [non-null] from person
...
select STRING_AGG(String,' ') as[sting] from DMLTable
...
SELECT
    ...
    ROUND(
        100.0 * SUM(CASE 
                        WHEN HIRE_DATE <= CURRENT_DATE - INTERVAL '3' YEAR 
                        THEN 1 ELSE 0 
                   END) / COUNT(*),
        2
...
select  min(missioncount)as[min mission ],max(missioncount)as[max mission]from Personal
...
SELECT 
  StudentID,
  FullName,
  Grade + COALESCE(LAG(Grade) OVER (ORDER BY StudentID), 0) AS UpdatedGrade
FROM Students;
...
WHERE EXISTS (
    SELECT 1
    FROM Student S2
    WHERE S2.StudentName <> Student.StudentName
      AND MONTH(S2.Birthday) = MONTH(Student.Birthday)
      AND DAY(S2.Birthday) = DAY(Student.Birthday)
);
